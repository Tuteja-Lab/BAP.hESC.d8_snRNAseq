...
# Figure 2: A, B and C
A = enhancedDimPlot(object = bapd8.integrated, grouping_var = 'ident', reduction = "umap", label = TRUE, pt.size = 1, alpha = 0.5) +
  ggtitle("A") + xlab("UMAP_1") + ylab("UMAP_2") +
  theme_classic() + 
  theme(legend.position = "none", plot.title = element_text(face = "bold"))

B <- enhancedDimPlot(object = bapd8.integrated, grouping_var = 'replicate', reduction = "umap", label = FALSE, pt.size = 1, alpha = 0.4) +
  ggtitle("B") +
  xlab("UMAP_1") +
  ylab("UMAP_2") +
  theme_classic() + 
...
C <- enhancedDimPlot(object = bapd8.integrated, grouping_var = 'orig.ident', reduction = "umap", label = FALSE, pt.size = 1, alpha = 0.4) +
  ggtitle("C") +
  xlab("UMAP_1") +
  ylab("UMAP_2") +
  theme_classic() + 
...
ggsave("Figure_2_A.svg", plot = A, dpi=900, width = 5, height = 5)
ggsave("Figure_2_B.svg", plot = B, dpi=900, width = 5, height = 5)
ggsave("Figure_2_C.svg", plot = C, dpi=900, width = 5, height = 5)
ABC = (A | B | C)
ggsave("Figure_2_ABC.svg", plot = ABC, dpi=900, width = 16, height = 6)
...
markers.all.1$avg_FC <- exp(markers.all.1$avg_logFC)
markers.all.2$avg_FC <- exp(markers.all.2$avg_logFC)
markers.all.3$avg_FC <- exp(markers.all.3$avg_logFC)
markers.all.4$avg_FC <- exp(markers.all.4$avg_logFC)
markers.all.5$avg_FC <- exp(markers.all.5$avg_logFC)
markers.all.6$avg_FC <- exp(markers.all.6$avg_logFC)
markers.all.7$avg_FC <- exp(markers.all.7$avg_logFC)
markers.all.8$avg_FC <- exp(markers.all.8$avg_logFC)
markers.all.9$avg_FC <- exp(markers.all.9$avg_logFC)
markers.filtered.1 <- markers.all.1 %>% filter(avg_FC >= 1.5) %>% filter(p_val_adj <= 0.05) %>% arrange(desc(avg_FC))
markers.filtered.2 <- markers.all.2 %>% filter(avg_FC >= 1.5) %>% filter(p_val_adj <= 0.05) %>% arrange(desc(avg_FC))
markers.filtered.3 <- markers.all.3 %>% filter(avg_FC >= 1.5) %>% filter(p_val_adj <= 0.05) %>% arrange(desc(avg_FC))
markers.filtered.4 <- markers.all.4 %>% filter(avg_FC >= 1.5) %>% filter(p_val_adj <= 0.05) %>% arrange(desc(avg_FC))
markers.filtered.5 <- markers.all.5 %>% filter(avg_FC >= 1.5) %>% filter(p_val_adj <= 0.05) %>% arrange(desc(avg_FC))
markers.filtered.6 <- markers.all.6 %>% filter(avg_FC >= 1.5) %>% filter(p_val_adj <= 0.05) %>% arrange(desc(avg_FC))
markers.filtered.7 <- markers.all.7 %>% filter(avg_FC >= 1.5) %>% filter(p_val_adj <= 0.05) %>% arrange(desc(avg_FC))
markers.filtered.8 <- markers.all.8 %>% filter(avg_FC >= 1.5) %>% filter(p_val_adj <= 0.05) %>% arrange(desc(avg_FC))
markers.filtered.9 <- markers.all.9 %>% filter(avg_FC >= 1.5) %>% filter(p_val_adj <= 0.05) %>% arrange(desc(avg_FC))
...
topmarkers <- function(number) {
  top.num.markers <- c(head(markers.filtered.names.1, number), head(markers.filtered.names.2, number), 
                       ...
                       head(markers.filtered.names.9, number))
...
runpce <- function(inputgenelist, clusternumber) {
  ...
  ggsave(paste("Cluster_", clusternumber, ".svg"), dpi=700, width = 10, height = 8)
}
