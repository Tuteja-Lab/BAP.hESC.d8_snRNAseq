...
The 10X data was already processed with `CellRanger` and the coutns table was ready for us to import for the data analyses.
...
```{r snrnaseqQC1, dpi = 150, fig.width=8, fig.height=8, fig.cap= "Relationship between the total molecules detected (transcripts) vs. total genes detected across samples. Each nucleus is represented as a dot, with the color intensity representing the mitochondrial read ratio in that nucleus."}
...
```{r snrnaseqQC2, dpi = 150, fig.width=6, fig.height=4, fig.cap= "Number of nucleus found in each sample" }
...
```{r snrnaseqQC3, dpi = 150, fig.width=6, fig.height=4, fig.cap="Density of nucleus vs. UMIs"  }
...
```{r snrnaseqQC4, dpi = 150, fig.width=6, fig.height=4, fig.cap="Nubmer of Nucleus vs. genes"}
...
```{r snrnaseqQC5, dpi = 150, fig.width=6, fig.height=4, fig.cap="Nubmer of Nuclei vs. genes" }
...
```{r snrnaseqQC6, dpi = 150, fig.width=6, fig.height=4 }
...
```{r filtering1, dpi = 150, fig.width=6, fig.height=4 }
...
```{r filtering2, dpi = 150, fig.width=8, fig.height=4.5 }
...
```{r filtering3, dpi = 150, fig.width=10, fig.height=5 }
...
```{r filtering4, dpi = 150, fig.width=10, fig.height=5 }
...
```{r filtering5, dpi = 150, fig.width=10, fig.height=5 }
...
```{r filtering6, dpi = 150, fig.width=8, fig.height=6 }
...
```{r dimplot1, dpi = 150, fig.width=6, fig.height=6, fig.cap="Fig2A: Dimensional reduction plot showing 5,355 nuclei plotted in two dimensions. The colored dots represent individual nuclei and are assigned based on cluster identity" }
...
```{r dimplot2, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig2B: Dimensional reduction plot showing 5,355 nuclei plotted in two dimensions. The colored dots represent individual nuclei and are assigned based on treatment." }
...
```{r dimplot3, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig2C: Dimensional reduction plot showing 5,355 nuclei plotted in two dimensions. The colored dots represent individual nuclei and are assigned based on replicate"}
...
Combined expression of all the markers genes in various clusters. The markers have higher expression in their respecitve cluster than compared to rest of the clusters.
...
```{r clusterstats, dpi = 150, fig.width=8, fig.height=5 }
...
Volcano Plots: This plots allows us to visualize the genes that are overexpressed in each condition along with its p-value. First, we will setup a function to make a volcano plot and then we call them for each cluster and depict them as an interactive plot.

```{r volcano, dpi = 150, fig.width=8, fig.height=6 }
...
```{r de.clus1, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig.6-1: Interactive Volcano plot showing genes overexpressed in 5% and 20% oxygen conditions for cluster 1" }
...
```{r de.clus2, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig.6-2: Interactive Volcano plot showing genes overexpressed in 5% and 20% oxygen conditions for cluster 2" }
...
```{r de.clus3, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig.6-3: Interactive Volcano plot showing genes overexpressed in 5% and 20% oxygen conditions for cluster 3" }
#ggplotly(myVolcanoPlot(clus3.five.twenty, 3))
myVolcanoPlot(clus3.five.twenty, 3)
```
### Volcano plot for cluster 4

```{r de.clus4, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig.6-4: Interactive Volcano plot showing genes overexpressed in 5% and 20% oxygen conditions for cluster 4" }
#ggplotly(myVolcanoPlot(clus4.five.twenty, 4))
myVolcanoPlot(clus4.five.twenty, 4)
```
### Volcano plot for cluster 5

```{r de.clus5, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig.6-5: Interactive Volcano plot showing genes overexpressed in 5% and 20% oxygen conditions for cluster 5" }
#ggplotly(myVolcanoPlot(clus5.five.twenty, 5))
myVolcanoPlot(clus5.five.twenty, 5)
```
### Volcano plot for cluster 6

```{r de.clus6, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig.6-6: Interactive Volcano plot showing genes overexpressed in 5% and 20% oxygen conditions for cluster 6" }
#ggplotly(myVolcanoPlot(clus6.five.twenty, 6))
myVolcanoPlot(clus6.five.twenty, 6)
```
### Volcano plot for cluster 7

```{r de.clus7, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig.6-7: Interactive Volcano plot showing genes overexpressed in 5% and 20% oxygen conditions for cluster 7" }
#ggplotly(myVolcanoPlot(clus7.five.twenty, 7))
myVolcanoPlot(clus7.five.twenty, 7)
```
### Volcano plot for cluster 8

```{r de.clus8, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig.6-8: Interactive Volcano plot showing genes overexpressed in 5% and 20% oxygen conditions for cluster 8" } 
#ggplotly(myVolcanoPlot(clus8.five.twenty, 8))
myVolcanoPlot(clus8.five.twenty, 8)
```
### Volcano plot for cluster 9

```{r de.clus9, dpi = 150, fig.width=8, fig.height=6, fig.cap="Fig.6-9: Interactive Volcano plot showing genes overexpressed in 5% and 20% oxygen conditions for cluster 9"}
...
```{r dataprep, dpi = 150, fig.width=8, fig.height=6 }
DefaultAssay(bapd8.integrated) <- "RNA"
Idents(bapd8.integrated) <- "new_clusters"
...
```{r fig3a, dpi = 150, fig.width=7, fig.height=3, fig.cap="Fig3A: Violin plots showing expression (average log fold change) for genes encoding transcription factors" }
...
```{r fig3b, dpi = 150, fig.width=7, fig.height=3, fig.cap="Fig3B: Violin plots showing expression (average log fold change) for genes encoding Structural proteins" }
fig3b <- c("KRT7", "KRT23")
...
```{r fig3c, dpi = 150, fig.width=7, fig.height=3, fig.cap="Fig3C: Violin plots showing expression (average log fold change) for genes encoding Hormones" }
fig3c <- c("CGA", "PGF")
...
```{r fig3d, dpi = 150, fig.width=7, fig.height=3, fig.cap="Fig3D: Violin plots showing expression (average log fold change) for genes encoding Transporters and Carcinoembryonic Antigen" }
...
```{r fig3e, dpi = 150, fig.width=7, fig.height=3, fig.cap="Fig3E: Violin plots showing expression (average log fold change) for genes encoding enzymes" }
...
```{r fig3f, dpi = 150, fig.width=7, fig.height=3, fig.cap="Fig3F: Violin plots showing expression (average log fold change) for genes encoding lncRNAs" }
...
```{r fig5a, fig.show='hold', dpi = 150, fig.width=6, fig.height=5, fig.cap="Fig5A: STB specific genes shared by clusters 5 and 6"}
...
```{r fig5bc, out.width=c('50%', '50%'), fig.show='hold', dpi = 150, fig.width=2, fig.height=5, fig.cap="Fig5: STB specific genes shared by clusters 5 and 6. Some highly expressed STB specific genes show (A) higher expression in cluster 5 and (B) higher expression in cluster 6" }
...
```{r figS4a, dpi = 150, fig.width=6, fig.height=3, fig.cap="Fig S4A: Violin plots showing low expression levels for mesoderm genes" }
...
```
```{r figS4b, dpi = 150, fig.width=6, fig.height=3, fig.cap="Fig S4B: Violin plots showing low expression levels for endoderm genes" }
multi_dittoPlot(bapd8.integrated, vars = endoderm, group.by = "new_clusters",
                       vlnplot.lineweight = 0.2, jitter.size = 0.3, ncol = 1, color.panel = color_list)
```

```{r figS4c, dpi = 150, fig.width=6, fig.height=3, fig.cap="Fig S4C: Violin plots showing low expression levels for ectoderm genes" }
...
```{r figS5, dpi = 150, fig.width=6, fig.height=8, fig.cap="Fig S5: Expression levels of Cellular Proliferation Markers across clusters" }
...
```{r figS6, out.width=c('33%', '33%', '33%'), fig.show='hold', dpi = 150, fig.width=2, fig.height=8, fig.cap="Fig S6: Expression levels of genes in Clusters 2 and 3."}
...
```{r figS7, dpi = 150, fig.width=5, fig.height=9, fig.cap="Fig S7: Expression differences of selected genes across treatments shown as violin plots." }
...
```{r figS8, dpi = 150, fig.width=8, fig.height=4, fig.cap="Fig S8: SOX2 localization in the dimensionality reduction plot  and its (B) expression  across clusters."}
